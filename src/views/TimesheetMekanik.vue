    ...
    <div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <table class="table table-striped white-background">
                    <tr v-for="(service, i) in services" :key="i" :class="{'finished': service.x_state.toUpperCase() == 'finished'.toUpperCase()}">
                        ...
                        <td v-if="service.x_state.toUpperCase() == 'unfinished'.toUpperCase()">
                            <button v-if="isStart('services', i) == false" class="btn btn-sm btn-warning" @click="start = `services:${i}`">Start</button> &nbsp;
                            <button v-if="isStart('services', i)" class="btn btn-sm btn-danger" @click="start = ``">Stop</button> &nbsp;
                            <button class="btn btn-sm btn-primary" @click="finishTask({id: service.id, x_duration: service.x_duration})">Finish</button>
                        </td>
                        <td v-else>
                ...
                <table class="table table-striped white-background">
                    <tr v-for="(sparepart, i) in spareparts" :key="i" :class="{'finished': sparepart.x_state.toUpperCase() == 'finished'.toUpperCase()}">
                        ...
                        <td v-if="sparepart.x_state.toUpperCase() == 'unfinished'.toUpperCase()">
                            <button v-if="isStart('spareparts', i) == false" class="btn btn-sm btn-warning" @click="start = `spareparts:${i}`">Start</button> &nbsp;
                            <button v-if="isStart('spareparts', i)" class="btn btn-sm btn-danger" @click="start = ``">Stop</button> &nbsp;
                            <button class="btn btn-sm btn-primary" @click="finishTask({id: sparepart.id, x_duration: sparepart.x_duration})">Finish</button>
                        </td>
                        <td v-else>
                ...
                <table class="table table-striped white-background">
                    <tr v-for="(kel, i) in keluhan" :key="i" :class="{'finished': kel.x_state.toUpperCase() == 'finished'.toUpperCase()}">
                        ...
                        <td v-if="kel.x_state.toUpperCase() == 'unfinished'.toUpperCase()">
                            <button v-if="isStart('keluhan', i) == false" class="btn btn-sm btn-warning" @click="start = `keluhan:${i}`">Start</button> &nbsp;
                            <button v-if="isStart('keluhan', i)" class="btn btn-sm btn-danger" @click="start = ``">Stop</button> &nbsp;
                            <button class="btn btn-sm btn-primary" @click="finishTask({id: kel.id, x_duration: kel.x_duration})">Finish</button>
                        </td>
                        <td v-else>
                ...
                <button :disabled="isDone == false" @click="finish" type="button" class="btn btn-primary btn-block">Selesai</button>
            </div>
        </div>
    </div>
</template>
...
export default class TimesheetMekanik extends Vue {
    ...
    public setDuration(type: string, el: any, i: number): void {
        let getSecondsOnLocalStorage: any    =   localStorage.getItem(el.id),
            ...
            totalTime               : any    =   (seconds >= getSecondsOnLocalStorage) ? seconds : getSecondsOnLocalStorage;
