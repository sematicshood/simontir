      ...
      <div class="col-lg-12">
        ...
        <table class="table table-striped white-background">
          <tr v-for="(service, i) in services">
            ...
            <td style="text-align: left;">{{ service.name.split(':')[1] }}</td>
            <td style="text-align: right;">
              <div class="btn-group">
                <button
                  @click="accept(service.id)"
                  type="button"
                  :class="{'btn': true, 'btn-default': true, 'btn-primary': service.x_status == 'accept'}"
                >
                  <i class="fa fa-check"></i>
                </button>
                <button
                  @click="id_select = service.id"
                  v-b-modal="'reject'"
                  type="button"
                  :class="{'btn': true, 'btn-default': true, 'btn-primary': service.x_status == 'reject'}"
                >
        ...
        <table class="table table-striped white-background">
          <tr v-for="(spar, i) in sparepart">
            <td style="width: 20px;">{{ i += 1 }}</td>
            <td style="text-align: left;">{{ spar.name.split(':')[1] }}</td>
            <td style="text-align: right;">
              <div class="btn-group">
                <button
                  @click="accept(spar.id)"
                  type="button"
                  :class="{'btn': true, 'btn-default': true, 'btn-primary': spar.x_status == 'accept'}"
                >
                  <i class="fa fa-check"></i>
                </button>
                <button
                  @click="id_select = spar.id"
                  v-b-modal="'reject'"
                  type="button"
                  :class="{'btn': true, 'btn-default': true, 'btn-primary': spar.x_status == 'reject'}"
                >
                  <i class="fa fa-close"></i>
                </button>
              </div>
        ...
        <table class="table table-striped white-background">
          <tr v-for="(kel, i) in keluhan" :key="i">
            <td style="width: 20px;">{{ i += 1 }}</td>
            <td style="text-align: left;">{{ kel.name.split(':')[1] }}</td>
            <td style="text-align: right;">
              <div class="btn-group">
                <button
                  @click="accept(kel.id)"
                  type="button"
                  :class="{'btn': true, 'btn-default': true, 'btn-primary': kel.x_status == 'accept'}"
                >
                  <i class="fa fa-check"></i>
                </button>
                <button
                  @click="id_select = kel.id"
                  v-b-modal="'reject'"
                  type="button"
                  :class="{'btn': true, 'btn-default': true, 'btn-primary': kel.x_status == 'reject'}"
                >
                  <i class="fa fa-close"></i>
                </button>
              </div>
  ...
  public load(): void {
    ...
    mekanik.getFinalSO(this.$route.params.id).then((res: any) => {
      ...
      this.services = res.data.results.tasks.filter((el: any) => {
        return (
          el.name.split(":")[0].split(" ")[1] === undefined &&
    ...
    if (serviceCek === 0 && keluhanCek === 0 && sparepartCek === 0) {
      mekanik
        .finishFinal({
          ...
          user_cuci: this.userWash
        })
        .then(() => {
          this.$router.push({ name: "list_final_check" });
        });
    } else {
      mekanik.unlock({ invoice: this.$route.params.id }).then(() => {
        this.$router.push({ name: "list_final_check" });
      });
    }
  }
